...
def write_modbus():
    ...
    sunrise_hour_write = connection.connection_plc().write_registers(0x000, int(weather_config.get_sunrise().split(":")[0]), unit=config.UNIT)
    sunrise_minutes_write = connection.connection_plc().write_registers(0x001, int(weather_config.get_sunrise().split(":")[1]), unit=config.UNIT)
    sunset_hour_write = connection.connection_plc().write_registers(0x002, int(weather_config.get_sunset().split(":")[0]), unit=config.UNIT)
    sunset_minutes_write = connection.connection_plc().write_registers(0x003, int(weather_config.get_sunset().split(":")[1]), unit=config.UNIT)
    current_temperature_write = connection.connection_plc().write_registers(0x004, int(weather_config.get_temperature()), unit=config.UNIT)
    local_hour_write = connection.connection_plc().write_registers(0x005, int(weather_config.get_local_time()[3].split(":")[0]), unit=config.UNIT)
    local_minutes_write = connection.connection_plc().write_registers(0x006, int(weather_config.get_local_time()[3].split(":")[1]), unit=config.UNIT)
